...
var createEntity = require('./resolver.js').createEntity;
...
Locale.prototype.addAST = function(ast) {
  ...
  if (this.isPseudo) {
    // TODO optimize
    ast = walkContent(ast, PSEUDO_STRATEGIES[this.id].translate);
  }

  var keys = Object.keys(ast);
  for (var i = 0, key; key = keys[i]; i++) {
    this.entries[key] = createEntity(key, ast[key], this.entries);
  }
};
