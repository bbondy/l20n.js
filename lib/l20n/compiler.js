...
define(function (require, exports, module) {
  ...
  function Expression(value, index) {
    ...
    if (!value) {
      ...
      return HashLiteral(value, index);
    }
  }
  ...
  function HashLiteral(node, index) {
    return function hashLiteral(ctxdata) {
      ...
      if (argValue === 0 && 'zero' in node) {
        return [node.zero];
      }
      if (argValue === 1 && 'one' in node) {
        return [node.one];
      }
      if (argValue === 2 && 'two' in node) {
        return [node.two];
      }
