  ...
  Context.prototype.get = function(id, ctxdata) {
    return this.getFromLocale(id, ctxdata);
  };
  ...
  Locale.prototype.loadResource = function(url, cb, type) {
    ...
    switch (type) {
      case 'ini':
        ...
        break;
      case 'json':
        io.loadJSON(url.replace('{{locale}}', 'en-US'),
                    this.addJSONResource.bind(this, cb));
        break;
      case 'properties':
  ...
  Locale.prototype.loadINI= function(url, cb) {
    ...
    io.load(url, function(err, source) {
      ...
      var resLoads = ini.resources.length;
      ...
      for (var i = 0; i < ini.resources.length; i++) {
        self.loadResource(ini.resources[i], onResourcesLoaded, 'properties');
      }
    });
  };
